...
func DetectTranscriber(cfg *config.Config) Transcriber {
	...
	if cfg.Voice.Riva.Enabled {
		if modelName := strings.TrimSpace(cfg.Voice.ModelName); modelName != "" {
			modelCfg, err := cfg.GetModelConfig(modelName)
			if err == nil {
				if protocol, _ := providers.ExtractProtocol(modelCfg.Model); protocol == "nvidia" && modelCfg.APIKey() != "" {
					return NewRivaTranscriber(modelCfg.APIKey(), cfg.Voice.Riva)
				}
			}
		}
		for _, mc := range cfg.ModelList {
			if protocol, _ := providers.ExtractProtocol(mc.Model); protocol == "nvidia" && mc.APIKey() != "" {
				return NewRivaTranscriber(mc.APIKey(), cfg.Voice.Riva)
			}
		}
	}
