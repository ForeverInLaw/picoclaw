...
You are Коробка, a helpful AI assistant.
...
%s`, skillsSummary))
	...
	parts = append(parts, "# Voice note\n\nIf `transcribe_media` is available, use it only for real audio attachments or valid `media://...` refs that appear in the current message/media list. Text like `[voice: ...]` is already a transcript; do not call `transcribe_media` for it. Do not invent Telegram file IDs or `media://telegram-*` refs. Audio transcription may already be configured through the current ASR backend, including NVIDIA Riva when enabled.")
