...
func (d *AgentDefaults) ShouldSendToolFeedback(peerKind, senderID string) bool {
	...
	if peerKind != "direct" || strings.TrimSpace(senderID) == "" {
		return false
	}
...
func matchesToolFeedbackSender(allowList FlexibleStringSlice, senderID string) bool {
	...
	if idx := strings.Index(rawSenderID, ":"); idx >= 0 && idx+1 < len(rawSenderID) {
		rawSenderID = strings.TrimSpace(rawSenderID[idx+1:])
	}
	for _, allowed := range allowList {
		allowed = strings.TrimSpace(allowed)
		if allowed == senderID || allowed == rawSenderID {
			return true
		}
	}
	return false
...
type TelegramConfig struct {
	...
	NameTriggers       FlexibleStringSlice    `json:"name_triggers,omitempty" yaml:"-"`
	ParticipantAliases map[string]string      `json:"participant_aliases,omitempty" yaml:"-"`
...
type VoiceConfig struct {
	...
	Riva              RivaVoiceConfig `json:"riva,omitempty"`
}
